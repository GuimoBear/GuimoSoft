    ...
    internal class Pipeline
    {
        ...
        public Pipeline(IReadOnlyList<Type> middlewareTypes, Type messageType)
        {
            ...
            if (!typeof(IMessage).IsAssignableFrom(messageType))
                throw new ArgumentException($"{messageType.Name} deve implementar a interface {nameof(IMessage)}");

            var middlewareType = typeof(IMessageMiddleware<>).MakeGenericType(messageType);
