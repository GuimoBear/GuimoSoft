        ...
        public async Task ProduceAsync<TMessage>(string key, TMessage message, CancellationToken cancellationToken = default) where TMessage : IMessage
        {
            ...
            foreach (var (busName, @switch, endpoint) in _messageTypeCache.Get(message.GetType()))
            {
                await _producerManager
                    ...
                    .ProduceAsync(key, message, @switch, endpoint, cancellationToken);
            }
        }

        private static void ValidateParameters<TMessage>(string key, TMessage message) where TMessage : IMessage
        {
            if (message is null)
